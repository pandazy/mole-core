...
export const replaceAll = (target: string) =>
  (replacement: string) =>
    (src: string): string => {
      ...
      return indices
        .map(
          (at: number, i) => {
            ...
            const suffix = i === indices.length - 1
              ? src.substring(at + targetLen)
              : '';
...
(replacementMap: ReplaceMap) =>
  (src: string): string => {
    return keys(replacementMap).reduce(
      (accumulatedResult: string, target) => {
